					...
					<tr>
						<td>
							...
							<div class="col-sm-3 col-xs-12 d-inline-table">
								...
								@if(empty($edit))
									<button type="button" class="btn btn-danger   rounded-3 btn-xs pull-right" id="pos-cancel">@lang('sale.cancel')</button>
								@else
									<button type="button" class="btn btn-danger   rounded-3 hide btn-xs pull-right" id="pos-delete">@lang('messages.delete')</button>
								@endif
							</div>
						</td>
					</tr>
					...
					<tr>
						<td>
							<div class="col-sm-2 col-xs-6 col-2px-padding">
								...
								<button type="button" 
									class="btn btn-warning btn-block   rounded-3 @if($pos_settings['disable_draft'] != 0) hide @endif" 
									id="pos-draft">@lang('sale.draft')</button>

								<button type="button" 
									class="btn btn-info btn-block   rounded-3" 
							...
							<div class="col-sm-3 col-xs-6 col-2px-padding">
								@if(!empty($pos_settings['show_credit_sale_button']))
									...
									<button type="button" 
									class="btn bg-purple btn-block   rounded-3 no-print pos-express-finalize" 
								...
								@else
									<button type="button" 
									class="btn bg-maroon btn-block   rounded-3 no-print @if(!empty($pos_settings['disable_suspend'])) pos-express-btn btn-lg @endif pos-express-finalize @if(!array_key_exists('card', $payment_types)) hide @endif" 
								...
								@if(empty($pos_settings['disable_suspend']))
									<button type="button" 
									class="btn bg-red btn-block   rounded-3 no-print pos-express-finalize" 
							...
							<div class="col-sm-4 col-xs-12 col-2px-padding">
								<button type="button" class="btn bg-navy  btn-block   rounded-3 btn-lg no-print @if($pos_settings['disable_pay_checkout'] != 0) hide @endif pos-express-btn" id="pos-finalize" title="@lang('lang_v1.tooltip_checkout_multi_pay')">
								<div class="text-center">
									<i class="fa fa-check" aria-hidden="true"></i>
							...
							<div class="col-sm-3 col-xs-12 col-2px-padding">
								<button type="button" class="btn btn-success btn-block   rounded-3 btn-lg no-print @if($pos_settings['disable_express_checkout'] != 0 || !array_key_exists('cash', $payment_types)) hide @endif pos-express-btn pos-express-finalize"
